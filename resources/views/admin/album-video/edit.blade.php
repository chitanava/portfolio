<x-admin.layout.app>
  ...
  <x-admin.page-header title="Edit {{ $video->title }}">
    ...
    onclick="Livewire.emit('delete', {
        action: '{{ route('admin.galleries.albums.videos.destroy', [$gallery->id, $album->id, $video->id]) }}',
        title: 'Are you sure you want to delete the Video?',
        body: 'This action will permanently remove video.'
      })">Delete video</label>
  </x-admin.page-header>

  <form action="{{ route('admin.galleries.albums.videos.update', [$gallery->id, $album->id, $video->id]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
