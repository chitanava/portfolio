<x-admin.layout.app>
  <x-slot name="title">Create Video</x-slot>
  ...
  <x-admin.page-header title="Create Video"/>

  <form action="{{ route('admin.galleries.albums.videos.store', [$gallery->id, $album->id]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="grid grid-cols-3 gap-8 items-start">
      ...
      <div class="card-body space-y-4">
        ...
        <div class="form-control w-full">
          ...
          <input type="text" name="url" value="{{ old('url') }}" id="url" class="input input-bordered" />
          @error('url')
            <p class="text-xs text-error px-1 pt-2">{{ $message }}</p>
          @enderror
        </div>
