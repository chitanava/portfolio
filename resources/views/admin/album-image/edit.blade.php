<x-admin.layout.app>
  ...
  <x-admin.page-header title="Edit {{ $image->title }}">
    ...
    onclick="Livewire.emit('delete', {
        action: '{{ route('admin.galleries.albums.images.destroy', [$gallery->id, $album->id, $image->id]) }}',
        title: 'Are you sure you want to delete the Image?',
        body: 'This action will permanently remove image.'
      })">Delete image</label>
  </x-admin.page-header>

  <form action="{{ route('admin.galleries.albums.images.update', [$gallery->id, $album->id, $image->id]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PUT')
